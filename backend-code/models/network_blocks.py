...
class DoubleConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, middle_channels, out_channels, dropout=False, p=0.2):
        ...
        # 将Dropout加在2个卷积层的中间
        layers = [SingleConvBlock(in_channels, middle_channels)]  # 3, 32
        if dropout:
            layers.append(nn.Dropout(p=p))
        layers.append(SingleConvBlock(middle_channels, out_channels))  # 32, 32
...
class AttentionBlock2d(nn.Module):
    ...
    def forward(self, x, gate):
        ...
        # 注意力系数乘原始的x
        # np.save('attention_coefficient_of_ag', attention_coefficient)
        output = attention_coefficient * origin_x
        output = self.end_transform(output)
        # np.save('output_of_ag', output)
        return output
